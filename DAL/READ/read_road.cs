...
{
    public class read_road
    {
        private const string excel_name = "小区内道路明细表";
        private const string table_name = "T_road";
        private const int start_row = 6;
        private read_excel re;
        private read_tools rt;

        public read_road(string path)
        {
            re = new read_excel();
            re.init(path);
            rt = new read_tools();
        ...
        /// <returns></returns>
        private Model.TTGWeb.road read_road_(int row)
        {
            Model.TTGWeb.road road = new Model.TTGWeb.road()
            {
            ...
            PropertyInfo[] road_info = road.GetType().GetProperties();
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (index != -1)
                {
                    ...
                    if (type.Equals("String"))
                        road_info[index].SetValue(road, re.replaceBlank(Convert.ToString(obj)), null);
                    else if (type.Equals("Int32"))
                        road_info[index].SetValue(road, Convert.ToInt32(obj), null);
                    else if (type.Equals("Decimal"))
                        road_info[index].SetValue(road, Convert.ToDecimal(obj), null);
                    else
                    {
        ...
        public void read_and_insert()
        {
            ...
            for (int i = start_row; i < max_row; i++)
            {
                ...
                if (!num.Equals("0"))
                {
                    if (!new DAL.TTGWeb.road().Add(read_road_(i)))
                        throw new Exception("道路明细表插入失败！");
                }
            }
        }
        /// <summary>
