...
//    获得弹窗对象
    ...
    if (!myCallout) {
        ...
        '<div class="basicInfo clearfix" style="height:70px;">' +
        '<div class="headImg"><img src="{{@url-img}}"  style="width:104px;height:69px;"/></div>' +
        '<div class="headInfo"><p>{{@address}}</p></div>' +
        '<div class="headInfo"><p onclick="SelectBuffer(\&#39;{2}\&#39;)" style="display:{{@showBuffer}}">查看服务半径</p>' +
        '</div></div>' +
        '<div class="comment"><a class="num" >具体介绍</a><p class="desc"><i class="arrow"></i>{{@content}}</p>' +
        '</div></div></div></div>' +
