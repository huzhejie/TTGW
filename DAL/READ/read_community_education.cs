...
{
    public class read_community_education
    {
        private const string excel_name = "小区教育及医疗基本情况";
        private read_excel re;
        private read_tools rt;
        private const int column2 = 6;

        public read_community_education(string path)
        {
            re = new read_excel();
            re.init(path);
            rt = new read_tools();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        private Model.TTGWeb.community_education read_edu(string id)
        {
            string table_name = "T_community_education";
            Model.TTGWeb.community_education cem = new Model.TTGWeb.community_education()
            {
                community_id = id,
            ...
            PropertyInfo[] edu_info = cem.GetType().GetProperties();
            for (int i = 0; i < list.Count; i++)
            {
                ...
                int index = rt.getIndex(attname, edu_info);
                if (index != -1)
                {
                    string type = edu_info[index].PropertyType.Name;
                    object obj = re.readcell(list[i].row_num, list[i].column_num);
                    if (type.Equals("String"))
                        edu_info[index].SetValue(cem, re.replaceBlank(Convert.ToString(obj)), null);
                    else if (type.Equals("Int32"))
                        edu_info[index].SetValue(cem, Convert.ToInt32(obj), null);
                    else if (type.Equals("Decimal"))
                        edu_info[index].SetValue(cem, Convert.ToDecimal(obj), null);
                    else
                    {
                        string in_type = Nullable.GetUnderlyingType(edu_info[index].PropertyType).ToString();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        private Model.TTGWeb.community_medical read_medical(string id)
        {
            const string table_name = "T_community_medical";
            Model.TTGWeb.community_medical cmm = new Model.TTGWeb.community_medical()
            {
                community_id = id,
                //statistics_date = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1)
                //statistics_date = Convert.ToDateTime("2013/1/1")
            ...
            PropertyInfo[] medical_info = cmm.GetType().GetProperties();
            for (int i = 0; i < list.Count; i++)
            {
                string attname = list[i].property;
                Model.TTGWeb.alter_field afm = new DAL.TTGWeb.alter_field().GetModel(attname, table_name);
                if (afm != null)
                    attname = afm.alter_field_name;
                int index = rt.getIndex(attname, medical_info);
                if (index != -1)
                {
                    string type = medical_info[index].PropertyType.Name;
                    object obj = re.readcell(list[i].row_column_num, 6);
                    if (type.Equals("String"))
                        medical_info[index].SetValue(cmm, re.replaceBlank(Convert.ToString(obj)), null);
                    else if (type.Equals("Int32"))
                        medical_info[index].SetValue(cmm, Convert.ToInt32(obj), null);
                    else if (type.Equals("Decimal"))
                        medical_info[index].SetValue(cmm, Convert.ToDecimal(obj), null);
                    else
                    {
                        string in_type = Nullable.GetUnderlyingType(medical_info[index].PropertyType).ToString();
                        if (in_type.Equals("System.Int32"))
                            medical_info[index].SetValue(cmm, Convert.ToInt32(obj), null);
                        else if (in_type.Equals("System.Decimal"))
                            medical_info[index].SetValue(cmm, Convert.ToDecimal(obj), null);
        ...
        {
            if (!new DAL.TTGWeb.community_education().Add(read_edu(id)))
                throw new Exception("小区教育表插入失败！");

            if (!new DAL.TTGWeb.community_medical().Add(read_medical(id)))
                throw new Exception("小区医疗表插入失败！");
        }
        /// <summary>
        /// 插入一行
        /// </summary>
        ...
        public void insert_line(int row, int column, string attname, string unit)
        {
            ...
            if (table_name.Equals("T_community_education"))
                afm.alter_field_name = "tceunset" + (count + 1);
            else if (table_name.Equals("T_community_medical"))
                afm.alter_field_name = "tcmunset" + (count + 1);
            if (!af.Add(afm))
